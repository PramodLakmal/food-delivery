...
const connectToRabbitMQ = async () => {
  try {
    ...
    // Create restaurant events exchange
...
const subscribeToEvents = async (exchange, routingPattern, callback) => {
  try {
    ...
    // Create queue for restaurant service
    const { queue } = await channel.assertQueue('restaurant_queue', { durable: true });
...
const publishEvent = async (routingKey, data) => {
  try {
    ...
    const exchange = 'restaurant_events';
